...
async def to_code(config):
    ...
    # Create the USBHIDDevice component directly (tinyusb descriptors are provided via `desc`).
    var = cg.new_Pvariable(config[CONF_ID])
    await cg.register_component(var, config)
    if CONF_KEYBOARD in report_id:
        keyboard = cg.new_Pvariable(config[CONF_KEYBOARD_ID], report_id[CONF_KEYBOARD])
